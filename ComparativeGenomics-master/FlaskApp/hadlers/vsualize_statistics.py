...
def clusterization(genes_count, features, clusterMethods, n_clusters, linkage):
    predictions = []
...
def buildPlots(data, methods, perplexity, clusterMethods, n_clusters, linkage):
    ...
    if len(strains) > 1:
        if len(features) > 1:
            ...
            x, predictions = clusterization(genes_count, features, clusterMethods, n_clusters, linkage)
...
def statistic_test(data, distance, tree, otu_ids, statMethods, matrix, perplexity, clusterMethods, n_clusters, linkage):
    genes_count = data.getCount()
    test_result = {}
    bc_dm, strains, features = matrix(genes_count, distance, tree, otu_ids)
    if len(bc_dm) != 0:
...
def matrix(genes_count, distance, tree, otu_ids):
    bc_dm = []
    if genes_count.is_empty():
        return bc_dm
    ...
    if len(strains) > 1:
        if len(features) > 1:
            if "0" in distance:
                bc_dm = beta_diversity("euclidean", genes_count_cut, strains)
            if "1" in distance:
                bc_dm = beta_diversity("braycurtis", genes_count_cut, strains)
            if "2" in distance:
                bc_dm = beta_diversity("weighted_unifrac", genes_count_cut, strains, tree=tree, otu_ids=otu_ids)
            if "3" in distance:
                bc_dm = beta_diversity("unweighted_unifrac", genes_count_cut, strains, tree=tree, otu_ids=otu_ids)
